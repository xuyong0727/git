...
class ScanApp:
    ...
    def start_system(self):
        ...
        # 在主线程快照所有 Tk 变量，避免后台线程访问 Tk 对象（非线程安全）
        try:
            self._snapshot_config = {
                'person': self.var_person.get(),
                'plc_addr': int(self.var_plc_addr.get()),
                'logic_delay': float(self.var_logic_delay.get()),
                'max_holes': int(self.var_max_holes.get()),
                'reg_read': int(self.var_reg_read.get()),
                'reg_write': int(self.var_reg_write.get()),
                'use_curve': bool(self.var_check_curve.get()),
                'use_waterproof': bool(self.var_check_waterproof.get()),
                'goods': self.var_goods.get().strip()
            }
        except Exception:
            # 兜底默认值
            self._snapshot_config = {
    ...
    def worker_thread(self):
        """后台工作线程"""
        try:
            # 获取配置 —— 使用在主线程快照的配置，避免从后台线程访问 Tk 变量
            snap = getattr(self, '_snapshot_config', None)
            if snap is None:
                person = self.var_person.get()
                ...
                goods_snapshot = self.var_goods.get().strip()
            else:
                person = snap.get('person', 'admin')
                plc_addr = snap.get('plc_addr', 1)
                logic_delay = snap.get('logic_delay', 0.01)
                max_holes = snap.get('max_holes', 4)
                reg_read = snap.get('reg_read', 541)
                reg_write = snap.get('reg_write', 542)
                use_curve = snap.get('use_curve', False)
                use_waterproof = snap.get('use_waterproof', False)
                goods_snapshot = snap.get('goods', '')
            ...
            while self.is_running:
                try:
                    ...
                    # 阶段 3: 校验
                    goods_name = goods_snapshot  # 使用启动时快照的 goods
