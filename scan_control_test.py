...
class ScanApp:
    ...
    def start_diagnostics(self, interval=1.0):
        """Start a lightweight diagnostics timer that records event-loop lag and process CPU time.
        Writes small diagnostic entries to a local `scan_diag.log` (via resource_path).
        This is low-overhead and safe to leave enabled in production for troubleshooting.
        """
        try:
            self._diag_interval = float(interval)
            self._diag_last = time.perf_counter()
            self._diag_proc_last = time.process_time()
            self._diag_file = resource_path("scan_diag.log")
            # write a header
            try:
                with open(self._diag_file, "a", encoding="utf-8") as f:
                    f.write(f"\n--- Diagnostic started {datetime.now().isoformat()} ---\n")
            except Exception:
                pass
            # schedule first tick
            self.root.after(int(self._diag_interval * 1000), self._diagnostics_tick)
        except Exception:
            pass
    ...
    def _diagnostics_tick(self):
        try:
            ...
            elapsed = now - getattr(self, '_diag_last', now)
            lag = elapsed - getattr(self, '_diag_interval', 1.0)
            cpu_delta = proc - getattr(self, '_diag_proc_last', proc)
            q_input = self.input_queue.qsize() if hasattr(self, 'input_queue') else -1
            q_log = self.log_queue.qsize() if hasattr(self, 'log_queue') else -1

            line = f"{datetime.now().isoformat()} lag={lag:.4f}s elapsed={elapsed:.4f}s cpu={cpu_delta:.4f}s q_in={q_input} q_log={q_log}\n"
            try:
                with open(getattr(self, '_diag_file', resource_path('scan_diag.log')), 'a', encoding='utf-8') as f:
                    f.write(line)
            except Exception:
                pass
            ...
            # schedule next tick
            try:
                self.root.after(int(getattr(self, '_diag_interval', 1.0) * 1000), self._diagnostics_tick)
            except Exception:
                pass
    ...
    def log(self, msg):
        ...
        try:
            self.log_queue.put_nowait(final_msg)
        except queue.Full:
            # 丢弃最旧的一条日志，避免长时间运行后日志队列挤占 UI 事件处理。
            try:
